  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans">
      ...
      <Navigation
        ...
        setActiveTab={(tab) => setCurrentView(tab as AppView)}
        userMode={userMode}
        ...
        onSelectVenue={(selectedVenue) => {
          ...
          storageService.saveVenue(selectedVenue);
        }}
        profiles={profiles}
        activeProfile={activeProfile}
        onSelectProfile={(selectedProfile) => {
          setActiveProfile(selectedProfile);
          storageService.setActiveProfile(selectedProfile);
        }}
        verificationState={verifications.length > 0 ? 'verified' : 'unverified'}
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
        {currentView === 'dashboard' && (
          <DashboardView
            ...
            userMode={userMode}
            onNavigate={(tab) => setCurrentView(tab as AppView)}
            onTriggerVerification={handleTriggerVerificationSweep}
            isCapturing={isCapturing}
            onToggleCapture={handleToggleCapture}
          />
        )}
